...
/// Built with nested `[>...<-]` loops to set cell values (idiomatic Brainfuck).
...
mod tests {
    ...
    #[test]
    fn hello_bf_world_uses_loops() {
        assert!(HELLO_BF_WORLD.contains('['));
        assert!(HELLO_BF_WORLD.contains(']'));
        // Naive linear-increment version was >1000 chars; loop version is compact.
        assert!(HELLO_BF_WORLD.len() < 500);
    }
